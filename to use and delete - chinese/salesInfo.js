! function($) {
    $.fn.salesInfo = function(options) {
        function reset() {
			...
			avd.html(""),
			ssd.html(""),
			bsd.html('<span class="empty">Fill in the details below to start calculating</span>'),
        ...
        function calculate() {
            ...
            avdtax = Math.ceil(avdtax), ssdtax = Math.ceil(ssdtax), bsdtax = Math.ceil(bsdtax), totalTax = avdtax + ssdtax + bsdtax, totalTax = Math.ceil(totalTax), totalTax = parseInt(totalTax).toFixed(2).replace(/./g, function(e, t, a) {
                return !t || "." === e || (a.length - t) % 3 ? e : "," + e
            }), dutyBox.html("$" + totalTax), avdtax = Math.ceil(avdtax), avdtax = parseInt(avdtax).toFixed(2).replace(/./g, function(e, t, a) {
                return !t || "." === e || (a.length - t) % 3 ? e : "," + e
            }), avdtax = avdtax + "<br />" + avdtext, avd.html("$" + avdtax), ssdtax = parseInt(ssdtax).toFixed(2).replace(/./g, function(e, t, a) {
                return !t || "." === e || (a.length - t) % 3 ? e : "," + e
            }), ssdtax = ssdtax + "<br />" + dText.ssd, bsd.html("$" + ssdtax), bsdtax = parseInt(bsdtax).toFixed(2).replace(/./g, function(e, t, a) {
                return !t || "." === e || (a.length - t) % 3 ? e : "," + e
            }), bsdtax = bsdtax + "<br />" + dText.bsd, ssd.html("$" + bsdtax), fadeBox.fadeIn()
        }

        function validateOptions() {
            return price = priceele.val().replace(/[^0-9\.]+/g, ""), 0 == price ? (priceele.addClass("error"), priceele.focus(), !1) : !0
        }
